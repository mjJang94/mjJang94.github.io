export const site = {
  name: '장민종',
  role: 'Android Engineer',
  tagline: '재현되지 않는 결함의 원인을 좁혀 해결하고, 지금 만들지 않을 것을 판단하는 일도 중요하게 생각합니다.',
  intro:
  manifest: [
    { key: 'experience', value: '8 years / Android' },
    { key: 'domain', value: 'Kids & Senior Safety' },
    { key: 'core', value: 'Kotlin, Compose, Coroutines' },
    { key: 'focus', value: 'Background reliability, Multi-module' },
    { key: 'location', value: 'Seoul, KR' },
  ],
  links: [
    { label: 'GitHub', href: 'https://github.com/mjJang94', handle: 'mjJang94' },
    { label: 'Velog', href: 'https://velog.io/@mj_log', handle: 'mj_log' },
    { label: 'Tistory', href: 'https://alswhddl10.tistory.com', handle: 'alswhddl10' },
    { label: 'Email', href: 'mailto:devjjang@naver.com', handle: 'devjjang@naver.com' },
  ],
  kindLabel: {
    product: '프로덕트',
    library: '라이브러리',
    side: '사이드 프로젝트',
  } as Record<string, string>,
};
